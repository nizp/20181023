...
class Home extends Component {
  ...
  componentDidMount(){
    ...
    this.setState({iw:this.refs.banner.offsetWidth});
  }
  start = (ev) => {
  ...
  render() {
    let {iw,disX,o} = this.state;
    let bannerlist = null;
    let list = null,
    len = null,
    ulW = null ;

    let {data:{data,banner}} = this.props;
    console.log(this.props)
    let arr = [];
    if(data){
      arr.push(...banner,...banner);
      len = arr.length;
      ulW = len * iw;
      ...
      list = data.map((item,i)=>{
        return (
          <li key={i} 
           onClick={()=>{this.props.getplay(item.hash)}}
          >
            <p>{item.filename}</p>
